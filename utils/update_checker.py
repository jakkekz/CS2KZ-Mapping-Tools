    ...
    def download_and_install_update(self):
        ...
        try:
            ...
            with open(batch_script, 'w') as f:
                ...
                f.write(f'if errorlevel 1 (\n')
                f.write(f'    echo Failed to start application with start command\n')
                f.write(f'    echo Error code: %%errorlevel%%\n')
                f.write(f'    echo Trying alternate method...\n')
                f.write(f'    "{current_exe}"\n')
                f.write(f'    if errorlevel 1 (\n')
                f.write(f'        echo Failed to start application with direct call\n')
                f.write(f'        echo Error code: %%errorlevel%%\n')
                f.write(f'        echo.\n')
                f.write(f'        echo Please try manually running: {current_exe}\n')
                f.write(f'        echo If the issue persists, re-download from GitHub releases\n')
                f.write(f'        pause\n')
                f.write(f'    )\n')
                f.write(f')\n')
